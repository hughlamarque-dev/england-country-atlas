...
function comparisonHTML(){
 ...
 let html='<table class="comparison-table"><caption>Area profiles · source periods differ</caption><thead><tr><th scope="col">Measure</th>'+rows.map(p=>'<th scope="col">'+esc(p.name)+'</th>').join('')+'</tr></thead><tbody>';
 const metrics={population_2025:{title:'Modelled population',unit:'residents',period:'WorldPop 2025',decimals:0},...NEEDS_METRICS};
...
function currentAtlasLink(){
 ...
 const key=!$('analysisPage').hidden?$('analysisMetric').value:primaryId?.replace('district_need_','');
...
async function restoreAtlasState(){
 ...
 if(metric&&METRICS[metric]){if(!$('analysisPage').hidden){$('analysisMetric').value=metric;renderAnalysis();}else if(view==='needs'&&NEEDS_METRICS[metric])choose('district_need_'+metric);else if(view==='population'&&['population_2025','density_km2'].includes(metric))choose(metric==='population_2025'?'district_population':'district_density');}
