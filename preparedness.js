...
async function initialisePreparedness(){
 const [data,foundations,census,resilience,pilotPartners]=await Promise.all([getJSON('data/preparedness.json'),getJSON('data/community_foundations.geojson'),getJSON('data/census2021.json'),getJSON('data/resilience_forums.json'),getJSON('data/volunteering-partners.json').catch(()=>({records:[]}))]);
...
function renderPartners(){
 const matches=filteredPartners();$('partnerCount').textContent=matches.length+' shown · '+foundationFeatures.length+' community foundations + '+resilienceData.records.length+' resilience forums + '+volunteeringPartners.length+' pilot-area organisations';$('partnersExport').disabled=!matches.length;const box=$('partnerCards');box.replaceChildren();
