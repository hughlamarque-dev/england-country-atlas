'use strict';
let censusData,resilienceData,partnerRecords=[],volunteeringPartners=[];
...
async function initialisePreparedness(){
 const [data,foundations,census,resilience,pilotPartners]=await Promise.all([getJSON('data/preparedness.json'),getJSON('data/community_foundations.geojson'),getJSON('data/census2021.json'),getJSON('data/resilience_forums.json'),getJSON('data/volunteering-partners.json?v=20260918.11').catch(()=>({records:[]}))]);
 volunteeringPartners=pilotPartners.records||[];
 ...
 partnerRecords=foundationFeatures.map((f,i)=>({...f.properties,source_region:f.properties.region,region:f.properties.region==='Yorkshire and Humber'?'Yorkshire and the Humber':f.properties.region,id:'foundation-'+i,feature:f,category:'Community foundation',source_url:data.partner_source,directory_updated:'Not stated',role:'Local charitable funding and community support',verification:'UKCF directory entry; funding eligibility and service coverage require confirmation.'})).concat(resilience.records.map((p,i)=>({...p,id:'lrf-'+i}))).sort((a,b)=>a.name.localeCompare(b.name));
 partnerRecords.push(...volunteeringPartners);partnerRecords.sort((a,b)=>a.name.localeCompare(b.name));
 $('partnerType').replaceChildren(new Option('All types',''));
 for(const type of [...new Set(partnerRecords.map(p=>p.category))].sort())$('partnerType').add(new Option(type,type));
...
function openBriefing(){
 ...
 $('downloadBrief').onclick=()=>{
  const html='<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>'+esc(p.name)+' · Area briefing</title><style>body{max-width:880px;margin:40px auto;padding:0 24px;font:15px/1.6 Arial,sans-serif;color:#253d39}h1{font-size:32px}h3{margin-top:28px}table{border-collapse:collapse;width:100%;font-size:13px}th,td{text-align:right;padding:9px;border-bottom:1px solid #d4dfda}th:first-child{text-align:left}.table-wrap{overflow-x:auto}.brief-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:12px}.brief-stat{padding:15px;background:#f1f5f2;border:1px solid #d4dfda}.brief-stat strong{display:block;font-size:26px}.brief-stat small{display:block}a{color:#185e52}.method-note{font-size:12px}@media print{body{margin:0}a{color:inherit}.brief-stat{break-inside:avoid}}@media(max-width:500px){.brief-grid{grid-template-columns:1fr}}</style><h1>'+esc(p.name)+' · Area briefing</h1>'+briefingHTML()+'</html>';
...
function renderPartners(){
 const matches=filteredPartners();$('partnerCount').textContent=matches.length+' shown · '+foundationFeatures.length+' community foundations + '+resilienceData.records.length+' resilience forums + '+volunteeringPartners.length+' volunteering / community organisations';$('partnersExport').disabled=!matches.length;const box=$('partnerCards');box.replaceChildren();
 ...
 for(const p of matches){const card=document.createElement('article');card.className='partner-card';card.innerHTML='<p class="eyebrow">'+esc(p.region)+' · '+esc(p.category)+'</p><h2>'+esc(p.name)+'</h2><p>'+esc(p.role)+'</p>'+(p.coverage_note?'<p class="partner-coverage"><strong>Area served:</strong> '+esc(p.coverage_note)+'</p>':'')+(p.address?'<p class="office-address">'+esc(p.address)+'</p>':'')+(p.phone?'<p><a href="tel:'+esc(p.phone.replace(/[^+0-9]/g,''))+'">'+esc(p.phone)+'</a></p>':'')+'<div class="partner-actions">'+(p.website&&/^https?:/.test(p.website)?'<a href="'+esc(p.website)+'" target="_blank" rel="noopener noreferrer">Website ↗</a>':'')+(p.risk_register?'<a href="'+esc(p.risk_register)+'" target="_blank" rel="noopener noreferrer">Risk register ↗</a>':'')+'<a href="'+esc(p.source_url)+'" target="_blank" rel="noopener noreferrer">Source ↗</a></div><details class="partner-provenance"><summary>Directory checked '+esc(p.checked)+'</summary><p>'+esc(p.verification)+'</p>'+(p.directory_updated!=='Not stated'?'<p>Source directory updated '+esc(p.directory_updated)+'.</p>':'')+(p.evidence?.length?'<ul>'+p.evidence.map(e=>'<li>'+esc(e.claim)+' <a href="'+esc(e.url)+'" target="_blank" rel="noopener noreferrer">Source ↗</a></li>').join('')+'</ul>':'')+'</details>';
 const actions=document.createElement('div');actions.className='partner-actions';
 if(p.area_ids?.length){const pilot=document.createElement('a');pilot.href='volunteering.html?area='+encodeURIComponent(p.area_ids[0]);pilot.textContent='Explore local volunteering →';actions.append(pilot);}
 if(p.volunteering_url&&/^https?:/.test(p.volunteering_url)){const route=document.createElement('a');route.href=p.volunteering_url;route.target='_blank';route.rel='noopener noreferrer';route.textContent='Volunteering enquiries ↗';actions.append(route);}
