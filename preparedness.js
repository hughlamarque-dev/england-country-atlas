...
async function initialisePreparedness(){
 const [data,foundations]=await Promise.all([getJSON('data/preparedness.json'),getJSON('data/community_foundations.geojson')]);
 preparednessData=data;foundationFeatures=foundations.features;
 const districts=layer('districts');
 for(const[key,meta]of Object.entries(NEEDS_METRICS)){
  manifest.analysis_metrics[key]={...meta,source:NEEDS_SOURCE};
  manifest.layers.push({...districts,id:'district_need_'+key,title:meta.title,group:'Community needs',date:'2025 release · corrected November 2025',source:'MHCLG · English Indices of Deprivation',source_url:NEEDS_SOURCE,unit:meta.unit,note:meta.note+' '+data.geography,count:Object.keys(data.records).length,style:{metric:key,breaks:meta.breaks,colors:NEEDS_COLORS}});
 }
 ...
 $('partnersTab').onclick=showPartners;$('partnerSearch').oninput=renderPartners;$('partnerRegion').onchange=renderPartners;
 for(const region of [...new Set(foundationFeatures.map(f=>f.properties.region))].sort()){$('partnerRegion').add(new Option(region,region));}
 $('partnersExport').onclick=()=>downloadTable(filteredPartners().map(f=>({...f.properties,longitude:f.geometry.coordinates[0],latitude:f.geometry.coordinates[1],source_url:preparednessData.partner_source,coverage_note:preparednessData.partner_note})),'England-community-foundations.csv');
...
function addAreaBriefing(){
 ...
 const button=document.createElement('button');button.className='wide-action primary-action';button.textContent='Open area briefing';button.onclick=openBriefing;extra.append(button);box.append(extra);
}
...
function briefingHTML(){
 ...
 for(const[key,meta]of Object.entries(NEEDS_METRICS))html+=metricCard(meta.title,n?fmt(n[key],1)+'%':'Unavailable',meta.unit+' · IoD2025');
...
function downloadTable(rows,name){if(!rows.length){toast('No matching records to export.');return;}const keys=Object.keys(rows[0]),cell=v=>'"'+String(v??'').replace(/^[=+@\-]/,"'$&").replaceAll('"','""')+'"';downloadBlob('\ufeff'+[keys,...rows.map(r=>keys.map(k=>r[k]))].map(r=>r.map(cell).join(',')).join('\r\n'),name,'text/csv;charset=utf-8');}
function filteredPartners(){const q=$('partnerSearch').value.trim().toLowerCase(),region=$('partnerRegion').value;return foundationFeatures.filter(f=>(!region||f.properties.region===region)&&(!q||[f.properties.name,f.properties.region,f.properties.address].join(' ').toLowerCase().includes(q)));}
...
function renderPartners(){
 const matches=filteredPartners();$('partnerCount').textContent=matches.length+' foundation offices · '+foundationFeatures.length+' in the England directory';const box=$('partnerCards');box.replaceChildren();
 if(!matches.length){box.textContent='No foundations match this search. Try a broader place name or region.';return;}
 for(const f of matches){const p=f.properties,card=document.createElement('article');card.className='partner-card';card.innerHTML='<p class="eyebrow">'+esc(p.region)+'</p><h2>'+esc(p.name)+'</h2><p>'+esc(p.address)+'</p><p>'+esc(p.phone)+'</p><div class="partner-actions"><a href="'+esc(p.website)+'" target="_blank" rel="noopener noreferrer">Website ↗</a><button>Show office on map</button></div>';card.querySelector('button').onclick=()=>{clearArea();setView('partners');map.setView([f.geometry.coordinates[1],f.geometry.coordinates[0]],11);L.popup().setLatLng([f.geometry.coordinates[1],f.geometry.coordinates[0]]).setContent(popup(layer('community_foundations'),f)).openOn(map);if(innerWidth<=1024)showPanel(false);};box.append(card);}
}
