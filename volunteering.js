...
(() => {
  ...
  function cardHTML(record) {
    ...
    return `<article class="route-card" id="route-${esc(record.id)}" style="--theme-color:${colour(record)}">
      ...
      <div class="card-bottom"><div class="card-actions">${external(actionURL(record),primaryAction(record),'button-link '+(f.state==='current'?'primary-link':''))}<button type="button" data-details="${esc(record.id)}">More details</button></div><div class="card-source">${external(record.source_url,'Source checked')}<br>${esc(dateLabel(record.checked_at))}${f.reason&&f.state==='current'?'<br>Review due today':''}</div></div>
    </article>`;
  }
  ...
  function openRecord(id) {
    ...
    const fact=(label,value,wide=false)=>{if(value==null||String(value).trim()==='')return '';return `<div${wide?' class="wide"':''}><dt>${esc(label)}</dt><dd>${esc(value)}</dd></div>`;};
    const hasPracticalDetails=[arr(r.employee_fit).length,r.duration,r.team_size,r.cost,r.accessibility,r.requirements,r.coverage_note,r.location?.label,r.starts_on,r.ends_on,r.application_deadline,r.availability_note].some(value=>Array.isArray(value)?value.length:Boolean(value&&String(value).trim()));
    $('recordBody').innerHTML=`<p class="card-kind">${esc(LABELS.kind[r.kind]||'Route')} · ${esc(areaNames(r))}</p><h2 id="recordTitle" class="record-title">${esc(r.title)}</h2><p class="organisation">${esc(r.organisation)}</p><div class="tag-row">${statusBadge(r)}${arr(r.themes).map(t=>`<span class="tag">${esc(LABELS.theme[t]||t)}</span>`).join('')}</div><p class="record-description">${esc(r.description)}</p>${r.resilience_relevance?`<p class="record-assessment"><strong>Why this could be useful</strong>${esc(r.resilience_relevance)}</p>`:''}${f.state!=='current'?`<p class="stale-note">${esc(f.reason)} The source link remains available for checking.</p>`:''}
      ...
      ${hasPracticalDetails?`<details class="record-details"><summary>Practical details</summary><dl class="record-facts">${fact('Volunteer format',arr(r.employee_fit).map(v=>LABELS.fit[v]||v).join(' · '))}${fact('Time commitment',r.duration)}${fact('Team size',r.team_size)}${fact('Costs or contributions',r.cost)}${fact('Accessibility',r.accessibility,true)}${fact('Requirements',r.requirements,true)}${fact('Area served',r.coverage_note,true)}${fact('Map location',r.location?`${LABELS.basis[r.location.basis]||'Location to confirm'} · ${r.location.label||areaNames(r)}`:'No map location recorded',true)}${r.starts_on?fact('Published start date',dateLabel(r.starts_on)):''}${r.ends_on?fact('Published end date',dateLabel(r.ends_on)):''}${r.application_deadline?fact('Application deadline',dateLabel(r.application_deadline)):''}${fact('Availability',r.availability_note,true)}</dl></details>`:'<p class="record-no-details">Further practical details are not provided by the source.</p>'}
      <details class="record-provenance"><summary>Sources &amp; review</summary><div class="provenance-body"><p>Source checked: <strong>${esc(dateLabel(r.checked_at))}</strong> · Review due: <strong>${esc(dateLabel(r.review_due))}</strong>${r.source_published?' · Source published: '+esc(dateLabel(r.source_published)):''}. Source checks refer to published information, not confirmation of live places with the host.</p><p>${external(r.source_url,'Official source')}${r.location?.source_url?' · '+external(r.location.source_url,'Location source'):''}${r.location?.geocode_source_url?' · '+external(r.location.geocode_source_url,'Postcode coordinate source'):''}</p>${arr(r.evidence).length?`<details><summary>Evidence behind this record</summary><ul>${arr(r.evidence).map(e=>`<li>${esc(e.claim)} ${external(e.url,'Source')}</li>`).join('')}</ul></details>`:''}</div></details>`;
